...
namespace eeeOpener
{
    internal class Program
    {
        static void Main(string[] args)
        {
            if (args.Length < 1)
            {
                Console.WriteLine("Usage: eeeOpener <input>");
                return;
            }
            string inputFile = args[args.Length - 1];

            string tmpDir = GetTemporaryDirectory();
            string outputFile = Path.Combine(tmpDir, Path.GetFileNameWithoutExtension(inputFile)+".ee");

            Process conversion = new Process();
            conversion.StartInfo.FileName = "eeeDecode";
